...
edge-assm -aws-region "${assm_region}" -ssm-id "${assm_path}" -s3-name "${s3_bucket_name}" -chain-name "${chain_name}" -chain-id "${chain_id}" -block-gas-limit "${block_gas_limit}" -premine "${premine}" -epoch-size "${epoch_size}" &